#Mutations violating the infinite site assumption(four states)
#run the function clade first
...
k<-length(tree$edge.length);k    #number of edges
Mtimes<-c()
Mnumber<-c()
...
for (i in 1:k) {
  ...
  T<-c()
  A<-c()
  while (sum(T)<l) {   #if total time does not exceed the edge length, then continue
    A<-T
    v<-rexp(1,theta/2)   #simulate the time until next mutation occurs
    T<-c(T,v)
    if (sum(T)>l) {     #if total time exceeds the edge length, then stop
      Mtimes[i]<-list(A)
      Mnumber[i]<-length(A)}
  }
}
...
edgeMut<-which(Mnumber>0); edgeMut    #which edges have mutations?
